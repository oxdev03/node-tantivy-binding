...
describe('Schema and Query Performance', () => {
  ...
  bench('Parse simple query', () => {
    const schema = new SchemaBuilder().addTextField('title', { stored: true }).addTextField('body').build()
  ...
  bench('Parse complex boolean query', () => {
    ...
    index.parseQuery('(title:programming OR body:coding) AND category:tech AND NOT deprecated', [
      'title',
      'body',
      'category',
    ])
