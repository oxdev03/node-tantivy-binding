...
impl TokenizerStatic {
    ...
    #[napi]
    pub fn ngram(min_gram: Option<u32>, max_gram: Option<u32>, prefix_only: Option<bool>) -> Tokenizer {
        Tokenizer {
...
fn parse_language(lang: &str) -> Result<tvt::Language> {
    match lang.to_lowercase().as_str() {
        ...
        _ => Err(Error::from_reason(format!("Unsupported language: {}", lang))),
    }
}
        ...
        let builder: tvt::TextAnalyzerBuilder = match &tokenizer.inner {
            ...
            TokenizerType::Regex { pattern } => tvt::TextAnalyzer::builder(
                tvt::RegexTokenizer::new(pattern).map_err(|e| {
                    Error::from_reason(format!("Invalid regex pattern: {}", e))
                })?,
            )
            .dynamic(),
            TokenizerType::Ngram {
    ...
    pub fn filter(&mut self, filter: &Filter) -> Result<Self> {
        if let Some(builder) = self.builder.take() {
            let new_builder: tvt::TextAnalyzerBuilder = match &filter.inner {
                FilterType::AlphaNumOnly => {
                    builder.filter_dynamic(tvt::AlphaNumOnlyFilter {})
                }
                FilterType::AsciiFolding => {
                    builder.filter_dynamic(tvt::AsciiFoldingFilter)
                }
                FilterType::LowerCaser => {
                    builder.filter_dynamic(tvt::LowerCaser)
                }
                FilterType::RemoveLong { length_limit } => builder.filter_dynamic(
                    tvt::RemoveLongFilter::limit(*length_limit as usize),
                ),
                FilterType::Stemmer { language } => {
                ...
                FilterType::CustomStopWord { stopwords } => builder
