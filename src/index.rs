...
use napi::{Error, Result, Status, JsUnknown};
...
impl IndexWriter {
    ...
    pub fn add_document(&mut self, doc: &Document) -> Result<u64> {
        ...
        let doc = tantivy::schema::document::TantivyDocument::convert_named_doc(&self.schema, named_doc)
    ...
    pub fn delete_all_documents(&mut self) -> Result<()> {
        self.inner()?.delete_all_documents().map_err(to_napi_error)?;
        Ok(())
    }
...
impl Index {
    ...
    pub fn register_tokenizer(&self, name: String, analyzer: &CrateTextAnalyzer) {
        self.index.tokenizers().register(&name, analyzer.analyzer.clone());
    }
}
...
impl Index {
    ...
    fn register_custom_text_analyzers(index: &tv::Index) {
        ...
        for (name, lang) in &analyzers {
            let an = tantivy::tokenizer::TextAnalyzer::builder(tantivy::tokenizer::SimpleTokenizer::default())
