...
impl Document {
    ...
    pub fn add_date(&mut self, field_name: String, value: String) -> Result<()> {
        let date = if let Ok(dt) = DateTime::parse_from_rfc3339(&value) {
            ...
            DateTime::from_timestamp(timestamp, 0).ok_or_else(|| {
                Error::new(Status::InvalidArg, "Invalid timestamp".to_string())
            })?
        } else {
    ...
    pub fn add_ip_addr(&mut self, field_name: String, value: String) -> Result<()> {
        let ip_addr = IpAddr::from_str(&value).map_err(|e| {
            Error::new(Status::InvalidArg, format!("Invalid IP address: {}", e))
        })?;
        match ip_addr {
    ...
    for i in 0..keys.get_array_length()? {
        ...
        let json_value = match value_js.get_type()? {
            ...
            ValueType::Number => {
                ...
                serde_json::Value::Number(serde_json::Number::from_f64(n.get_double()?)
            ...
            _ => return Err(Error::new(Status::InvalidArg, "Unsupported value type".to_string())),
        };
...
fn js_unknown_to_json_value(value: JsUnknown) -> Result<serde_json::Value> {
    match value.get_type()? {
        ValueType::String => {
            ...
            Ok(serde_json::Value::String(s.into_utf8()?.as_str()?.to_string()))
        }
        ValueType::Number => {
            let n: JsNumber = unsafe { value.cast() };
            Ok(serde_json::Value::Number(serde_json::Number::from_f64(n.get_double()?)
                .ok_or_else(|| Error::new(Status::InvalidArg, "Invalid number".to_string()))?))
        }
        ValueType::Boolean => {
        ...
        _ => Err(Error::new(Status::InvalidArg, "Unsupported value type".to_string())),
    }
}
...
fn extract_value_for_type(
    ...
    let _type_error = |expected: &str| -> Error {
        Error::new(
            Status::InvalidArg,
            format!("Expected {} type for field {}, got another type", expected, field_name),
        )
    };
...
fn value_to_js(env: Env, value: &Value) -> Result<JsUnknown> {
    Ok(match value {
        ...
        Value::Date(d) => {
            ...
            let formatted = d.into_utc().format(&time::format_description::well_known::Iso8601::DEFAULT)
