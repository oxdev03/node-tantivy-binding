...
use napi::{bindgen_prelude::*, Error, Result, Status, JsNumber, ValueType};
...
fn js_object_to_json_value(obj: Object) -> Result<serde_json::Value> {
    ...
    for i in 0..keys.get_array_length()? {
        ...
        let json_value = match value_js.get_type()? {
            ValueType::String => {
                let s: String = value_js.coerce_to_string()?.into_utf8()?.as_str()?.to_string();
...
pub(crate) fn extract_value(value: &Unknown) -> Result<Value> {
    match value.get_type()? {
        ...
        _ => Err(Error::new(Status::InvalidArg, format!("Unsupported value type: {:?}", value.get_type())))
    }
}
...
pub(crate) fn extract_value_for_type(
    ...
    let error_msg = |type_name: &str| {
        format!("Expected {} type for field {}, got unexpected value", type_name, field_name)
    };
    ...
    match tv_type {
        ...
        tv::schema::Type::Date => {
            match value.get_type()? {
                ValueType::Number => {
                    ...
                    Ok(Value::Date(tv::DateTime::from_timestamp_secs(timestamp / 1000)))
                }
                ValueType::String => {
                    // Handle ISO date strings
                    let date_str = value.coerce_to_string()?.into_utf8()?.into_owned()?;
                    if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(&date_str) {
                        Ok(Value::Date(tv::DateTime::from_timestamp_secs(dt.timestamp())))
                    } else {
                        Err(Error::new(Status::InvalidArg, format!("Invalid ISO date string: {}", date_str)))
                    }
                }
        ...
        tv::schema::Type::Bytes => {
            ...
            if !value.is_buffer()? {
                return Err(Error::new(Status::InvalidArg, "Expected Buffer for bytes field"));
            }
...
fn deserialize_datetime<'de, D>(
    deserializer: D,
) -> std::result::Result<tv::DateTime, D::Error>
where
    D: Deserializer<'de>,
{
...
impl Document {
    ...
    pub fn extend(&mut self, env: Env, js_obj: Object, schema: Option<&Schema>) -> Result<()> {
        Document::extract_js_values_from_object(
            env,
            &js_obj,
            schema,
